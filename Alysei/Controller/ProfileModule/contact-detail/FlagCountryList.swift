...
class Country {
    ...
    init(name: String, code: String, callingCode: String) {
        self.name = name
        self.code = code
...
class FlagCountryList: UIViewController {

    @IBOutlet var tableView: UITableView!
    @IBOutlet var closeButton: UIButton!

    var countryList = [Country]()
    var didSelectCountry: ((_ country: Country) -> Void)?
...
extension FlagCountryList: UITableViewDelegate {
...
extension FlagCountryList: UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return countryList.count
    }

    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as? FlagCountryListCell {
            let country = countryList[indexPath.row]
            cell.name.text = "\(country.name)"
            cell.code.text = "+\(country.callingCode)"
            cell.flag.image = UIImage(named: country.name.lowercased())
            return cell
        }
...
func parseJSONtoCountry(_ fileName: String, fileExtension: String) -> [Country] {
    var countryArray = [Country]()
    if let directory = Bundle.main.path(forResource: fileName, ofType: fileExtension) {
        //TODO: Try JSON decodable with local json file.
        do {
            if let content = FileManager.default.contents(atPath: directory) {
                let jsonResponse = try JSONSerialization.jsonObject(with: content, options: .allowFragments)
                guard let json = jsonResponse as? [String: Any] else {
                    return []
                }
                guard let jsonArray = json["codes"] as? [[String: Any]]  else {
                    return []
                }
                for element in  jsonArray {
                    ...
                    let callingCode = element["callingCode"] as? String ?? ""
                    let country = Country(name: name, code: code, callingCode: callingCode)
