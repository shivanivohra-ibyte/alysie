...
extension UICollectionViewCell{
  ...
  func drawCollectionViewShadow() -> Void{
    
    self.contentView.makeCornerRadius(radius: 10.0)
    self.contentView.layer.borderWidth = 1.0
    self.contentView.layer.borderColor = UIColor.clear.cgColor
    self.contentView.layer.masksToBounds = false
    self.layer.shadowColor = AppColors.liteGray.color.cgColor
...
extension UIAlertController {
  ...
  convenience init(style: UIAlertController.Style, source: UIView? = nil, title: String? = nil, message: String? = nil, tintColor: UIColor? = nil) {
    ...
    let root = UIApplication.shared.windows.filter {$0.isKeyWindow}.first?.rootViewController?.view //UIApplication.shared.keyWindow?.rootViewController?.view
